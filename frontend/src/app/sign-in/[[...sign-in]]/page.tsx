...
import { Github, Eye, EyeOff, Mail, FileText, Lock } from "lucide-react";
...
export default function SignUpPage() {
          ...
          message: (
            <div className="p-4 text-center">
              ...
              <div className="text-destructive mt-2 mb-2">
                {data?.message || "Failed to signing in."}
              </div>
            </div>
          ),
        });
      ...
      setModal({
        ...
        message: (
          <div className="p-4 text-center">
            <div className="font-semibold text-primary">{values.email}</div>
            <div className="text-destructive mt-2 mb-2">
              {error?.message || "Error during signing in. Please try again."}
            </div>
          </div>
        ),
      });
  ...
  const handleGoogleLogin = () => {
    window.location.href = "http://localhost:4000/auth/google/sign-in";
  };
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="relative">
                        {/* Use "password" type always for Input to avoid showing the default eye toggle of browsers */}
                        <Input
                        ...
                        {/* Custom eye icon for show/hide */}
                        <button
                          type="button"
                          onClick={() => setShowPassword((prev) => !prev)}
                          className="absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-primary transition-colors"
                          tabIndex={-1}
                          // Prevent default password manager eye icon appearing (handled above with autoComplete)
                        >
                          {showPassword ? (
